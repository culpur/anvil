...
import { AnvilProcess, AnvilResponse } from './anvilProcess';
...
// ---------------------------------------------------------------------------
export class AnvilChatProvider implements vscode.WebviewViewProvider {
  public static readonly viewId = 'anvil.chatView';

  private view?: vscode.WebviewView;
  private anvilProc: AnvilProcess | null = null;
  ...
  async sendCodeAction(prompt: string): Promise<void> {
  ...
  private async ensureProcess(): Promise<void> {
    ...
    this.anvilProc = new AnvilProcess(path, model, provider);

    this.anvilProc.on('partial', (line: string) => {
      this.post({ type: 'partial', text: line });
    });

    this.anvilProc.on('response', (resp: AnvilResponse) => {
      if (resp.type === 'text') {
    ...
    this.anvilProc.on('exit', (_code: number | null) => {
    ...
    this.anvilProc.on('error', (err: Error) => {
  ...
  private async handleUserMessage(text: string, showInChat = false): Promise<void> {
    if (!text.trim()) { return; }
  ...
  private postStatus(status: 'connected' | 'connecting' | 'disconnected' | 'error'): void {
...
// ---------------------------------------------------------------------------
// Types for messages across the webview boundary
// ---------------------------------------------------------------------------
interface WebviewMessage {
  type: 'send' | 'clear' | 'ready' | 'restart';
  text: string;
}

interface OutboundMessage {
  type: 'user' | 'partial' | 'response' | 'done' | 'error' | 'system' | 'status' | 'clear';
  text?: string;
