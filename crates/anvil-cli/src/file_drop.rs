...
// ---------------------------------------------------------------------------
// Clipboard image support (Feature 8)
// ---------------------------------------------------------------------------

/// Attempt to read an image from the system clipboard.
///
/// On macOS we use `pngpaste` (if installed) or fall back to `pbpaste`.
/// On Linux we try `xclip -selection clipboard -t image/png -o`.
///
/// Returns `Some(FileDropResult)` when an image was found, `None` otherwise.
pub fn read_clipboard_image() -> Option<FileDropResult> {
    let tmpdir = std::env::temp_dir();
    let ts = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let tmp_path = tmpdir.join(format!("anvil_clipboard_{ts}.png"));

    let captured = if cfg!(target_os = "macos") {
        // pngpaste is a small homebrew tool that writes clipboard PNG to a file.
        ...
        let ok = std::process::Command::new("xclip")
            ...
            .stdout(std::fs::File::create(&tmp_path).ok()?)
            .status()
            .ok()
            .map(|s| s.success())
            .unwrap_or(false);
    ...
    if !captured || !tmp_path.exists() || tmp_path.metadata().ok()?.len() == 0 {
        let _ = fs::remove_file(&tmp_path);
        return None;
    }

    let result = process_file(&tmp_path);
    let _ = fs::remove_file(&tmp_path);
