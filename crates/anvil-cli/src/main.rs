...
fn anvil_config_json_exists() -> bool {
...
fn wizard_box_line(content: &str) {
    // Inner width is 56 characters (matching the 60-char outer box less 2
    // border chars and 2 padding spaces on each side).
    const INNER: usize = 56;
    let padded = format!("{content:<INNER$}");
    println!("\x1b[36m\u{2551}\x1b[0m  {padded}  \x1b[36m\u{2551}\x1b[0m");
}

/// Print a full-width horizontal separator using the given corner/bar chars.
fn wizard_box_top() {
    println!(
...
fn wizard_step_header(step: u8, total: u8, title: &str) {
    println!();
    println!(
        "\x1b[1;33mStep {step} of {total}: {title}\x1b[0m"
    );
...
/// needed.
fn wizard_save_credential(key: &str, value: &str) -> io::Result<()> {
...
/// Save the wizard result to `~/.anvil/config.json`.
fn wizard_save_config(config: &serde_json::Map<String, serde_json::Value>) -> io::Result<PathBuf> {
    let home = std::env::var("HOME").unwrap_or_default();
    let dir = PathBuf::from(home).join(".anvil");
    fs::create_dir_all(&dir)?;
    let path = dir.join("config.json");
    // Merge with any existing config so we don't wipe keys set elsewhere.
...
fn run_first_run_wizard() {
    ...
    // Track which providers were configured and their default model candidates.
    ...
    match ollama_choice.to_ascii_lowercase().as_str() {
        ...
        "2" => {
            ...
            match wizard_test_ollama(&url) {
                ...
                Err(e) => {
                    println!("\x1b[31m failed ({e})\x1b[0m");
                    println!("  Connection failed. Configuration saved; you can retry with `anvil login ollama`.");
                    // Still save the custom URL even when the test fails, so
        ...
        "3" => {
            ...
            print!("  Testing connection to {url}... ");
            let _ = io::stdout().flush();
            // Quick connectivity test (ignoring the key for the test itself).
            match wizard_test_ollama(&url) {
                Ok(models) => {
                    println!("\x1b[32m connected\x1b[0m");
                    configured_providers.push("ollama".to_string());
    ...
    match anthropic_choice.to_ascii_lowercase().as_str() {
        "1" | "" => {
            ...
            match run_anthropic_login() {
                Ok(()) => {
                    ...
                    model_candidates.push(("claude-opus-4-6".to_string(), "Anthropic".to_string()));
                    model_candidates.push(("claude-sonnet-4-6".to_string(), "Anthropic".to_string()));
                    model_candidates.push(("claude-haiku-4-5-20251213".to_string(), "Anthropic".to_string()));
                }
                Err(e) => {
        ...
        "2" => {
            ...
            if api_key.is_empty() {
                ...
                // Validate by hitting the models list endpoint.
    ...
    let provider_priority: Vec<String> = if configured_providers.is_empty() {
        // No providers configured — fall back to anthropic as default.
        ...
        for (i, p) in configured_providers.iter().enumerate() {
            let first_model = model_candidates
                .iter()
                .find(|(_, label)| label.to_ascii_lowercase().starts_with(&provider_label(p).to_ascii_lowercase()))
                .map(|(m, _)| m.as_str())
                .unwrap_or("—");
    ...
    let ollama_url_val = ollama_url.clone().unwrap_or_else(|| "http://localhost:11434".to_string());
