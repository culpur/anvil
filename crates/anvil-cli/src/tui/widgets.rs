...
pub(super) fn third_level_completions(command: &str, subcommand: &str) -> Vec<CompletionItem> {
    match (command, subcommand.trim()) {
        ...
        ("/configure", "statusline") | ("/configure", "status-line") | ("/configure", "status_line") => vec![
            ...
            CompletionItem { insert: "gaming".into(), hint: "\u{1f3ae} Emoji-rich gaming vibe + burn rate".into() },
            CompletionItem { insert: "devops".into(), hint: "\u{1f433} DevOps/SRE — MCP, permissions, RC".into() },
            CompletionItem { insert: "budget-tracker".into(), hint: "\u{1f4b8} Full cost breakdown — daily/weekly/monthly".into() },
            CompletionItem { insert: "zen".into(), hint: "\u{1f9d8} Ultra-minimal — model + context only".into() },
            CompletionItem { insert: "academic".into(), hint: "\u{1f4da} Learning — productivity + session time".into() },
            CompletionItem { insert: "hacker".into(), hint: "\u{1f480} Cyberpunk — permissions, RC, MCP".into() },
            CompletionItem { insert: "night-owl".into(), hint: "\u{1f319} Chill — time, session, relaxed".into() },
            CompletionItem { insert: "dashboard".into(), hint: "\u{1f4ca} Maximalist — every widget, 4 lines".into() },
        ],
