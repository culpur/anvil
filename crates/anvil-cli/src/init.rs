...
const GITIGNORE_ENTRIES: [&str; 2] = [".anvil/settings.local.json", ".anvil/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let anvil_md = cwd.join("ANVIL.md");
    let content = render_init_anvil_md(cwd);
    artifacts.push(InitArtifact {
        name: "ANVIL.md",
...
pub(crate) fn render_init_anvil_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# ANVIL.md".to_string(),
        String::new(),
        "This file provides guidance to Anvil (culpur.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Do not overwrite existing `ANVIL.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_anvil_md};
    use std::fs;
    use std::path::Path;
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert_eq!(
            fs::read_to_string(root.join(".anvil.json")).expect("read anvil json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        let anvil_md = fs::read_to_string(root.join("ANVIL.md")).expect("read anvil md");
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("ANVIL.md"), "custom guidance\n").expect("write existing anvil md");
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_anvil_md(Path::new(&root));
