...
async fn provider_client_dispatches_api_requests() {
    ...
    let client = match client {
        ProviderClient::AnvilApi(client) => {
            ProviderClient::AnvilApi(client.with_base_url(server.base_url()))
        }
...
async fn live_stream_smoke_test() {
    ...
    let mut stream = client
        .stream_message(&MessageRequest {
            model: std::env::var("ANVIL_MODEL").unwrap_or_else(|_| "claude-sonnet-4-6".to_string()),
            max_tokens: 32,
