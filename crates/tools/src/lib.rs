...
use runtime::{
    edit_file, execute_bash, glob_search, grep_search, load_system_prompt, memory_dir_for_project,
    read_file, write_file, ApiClient, ApiRequest, AssistantEvent, BashCommandInput, ContentBlock,
    ConversationMessage, ConversationRuntime, CronManager, GrepSearchInput, MessageRole,
    PermissionMode, PermissionPolicy, RuntimeError, Session, TaskManager, TeamManager, TokenUsage,
...
pub fn execute_tool(name: &str, input: &Value) -> Result<String, String> {
    ...
    if PLAN_MODE.load(Ordering::SeqCst) && name != "ExitPlanMode" {
    ...
    match name {
        ...
        "TeamAddMember" => from_value::<TeamAddMemberInput>(input).and_then(run_team_add_member),
        "TeamRemoveMember" => {
            from_value::<TeamRemoveMemberInput>(input).and_then(run_team_remove_member)
        }
        "TeamList" => run_team_list(),
        "TeamDelegate" => from_value::<TeamDelegateInput>(input).and_then(run_team_delegate),
...
// ---------------------------------------------------------------------------
// Task management tool runner functions
// ---------------------------------------------------------------------------
...
fn run_task_list() -> Result<String, String> {
    let mgr = TaskManager::global()
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
...
fn execute_web_search(input: &WebSearchInput) -> Result<WebSearchOutput, String> {
    ...
    // When ANVIL_WEB_SEARCH_BASE_URL is set (e.g. in tests) fall back to the
    ...
    let provider_override = input.provider.as_deref();

    let search_results = if input.search_all.unwrap_or(false) {
        engine.search_all(&input.query)
    } else {
        engine.search(&input.query, provider_override)
    }?;

    let mut hits: Vec<SearchHit> = search_results
        .iter()
        .map(|r| SearchHit {
            title: r.title.clone(),
            url: r.url.clone(),
...
fn resolve_skill_path(skill: &str) -> Result<std::path::PathBuf, String> {
    ...
    // Search CWD-relative .anvil/skills first (project-level skills)
...
fn is_builtin_subagent_type(normalized: &str) -> bool {
    matches!(
        normalized,
        "general-purpose" | "Explore" | "Plan" | "Verification" | "anvil-guide" | "statusline-setup"
    )
}

/// Search ancestor `.anvil/agents/` and `.codex/agents/` directories for a user-defined
/// agent definition matching `name`. Returns the parsed definition if found.
fn find_user_agent_def(name: &str) -> Option<UserAgentDef> {
    let cwd = std::env::current_dir().ok()?;

    // Collect candidate directories by walking up the tree
    let mut candidate_dirs: Vec<std::path::PathBuf> = cwd
        .ancestors()
        .flat_map(|ancestor| {
            [
    ...
    // Also check user-level homes
    if let Ok(codex_home) = std::env::var("CODEX_HOME") {
        let p = std::path::PathBuf::from(codex_home).join("agents");
        if p.is_dir() {
...
/// Parse `key = ["a", "b"]` style array from minimal TOML.
...
fn parse_agent_md(contents: &str) -> UserAgentDef {
    ...
    if first != Some("---") {
        // No frontmatter — entire contents become system_prompt
    ...
    for line in lines {
        if in_frontmatter {
            ...
            // Parse simple `key: value` or `key: [a, b]` YAML lines
            if let Some((key, value)) = line.split_once(':') {
                ...
                match key {
                    ...
                    "tools" => {
                        // Inline array: [Read, Grep] or ["Read", "Grep"]
                        if let Some(inner) =
    ...
    F: FnOnce(AgentJob) -> Result<(), String>,
{
    ...
    // Build system prompt, then prepend user-defined agent system_prompt if present
...
fn run_cron_list() -> Result<String, String> {
...
#[derive(Debug, Deserialize)]
struct TeamRemoveMemberInput {
    team_id: String,
    member_name: String,
}

#[derive(Debug, Deserialize)]
struct TeamDelegateInput {
    team_id: String,
    member_name: String,
    prompt: String,
}
...
#[allow(clippy::needless_pass_by_value)]
fn run_team_add_member(input: TeamAddMemberInput) -> Result<String, String> {
    let mut mgr = TeamManager::global()
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    mgr.add_member(
...
#[allow(clippy::needless_pass_by_value)]
fn run_team_remove_member(input: TeamRemoveMemberInput) -> Result<String, String> {
    let mut mgr = TeamManager::global()
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
...
#[allow(clippy::needless_pass_by_value)]
fn run_team_delegate(input: TeamDelegateInput) -> Result<String, String> {
    let mut mgr = TeamManager::global()
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
...
///
/// This is a thin wrapper around [`runtime::memory_dir_for_project`] exposed
/// here so callers in the tools layer do not need to import the runtime crate
/// directly.
#[must_use]
...
fn run_enter_plan_mode() -> Result<String, String> {
...
fn run_exit_plan_mode() -> Result<String, String> {
