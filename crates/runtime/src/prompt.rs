...
/// Minimum QMD score required for a result to be included in the system prompt.
...
impl SystemPromptBuilder {
    ...
    /// Search QMD for documents relevant to `user_message` and attach the top
    /// results as workspace knowledge.  Silently skips if QMD is unavailable.
    #[must_use]
    pub fn with_qmd_context(mut self, qmd: &QmdClient, user_message: &str) -> Self {
        if qmd.is_enabled() && !user_message.trim().is_empty() {
            self.qmd_results = qmd.search(user_message, QMD_MAX_RESULTS, QMD_MIN_SCORE);
        }
        self
    }

    /// Attach pre-fetched QMD results directly (useful when the search was run
    /// ahead of time to avoid blocking the prompt build path).
    #[must_use]
...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("ANVIL.md"),
            dir.join("ANVIL.local.md"),
            dir.join(".anvil").join("ANVIL.md"),
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Anvil instructions".to_string()];
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".anvil")).expect("nested anvil dir");
        fs::write(root.join("ANVIL.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("ANVIL.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".anvil")).expect("apps anvil dir");
        fs::write(root.join("apps").join("ANVIL.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".anvil").join("instructions.md"),
            "apps dot anvil instructions",
        )
        .expect("write apps dot anvil instructions");
        fs::write(nested.join(".anvil").join("ANVIL.md"), "nested rules")
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("ANVIL.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("ANVIL.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.anvil/ANVIL.md")),
            "ANVIL.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("ANVIL.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? ANVIL.md"));
    ...
    fn load_system_prompt_reads_anvil_files_and_config() {
        ...
        fs::write(root.join("ANVIL.md"), "Project rules").expect("write instructions");
        fs::write(
...
",
    ...
    #[test]
    fn renders_anvil_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".anvil")).expect("anvil dir");
        fs::write(root.join("ANVIL.md"), "Project rules").expect("write ANVIL.md");
        fs::write(
            root.join(".anvil").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# Anvil instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_anvil_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".anvil")).expect("nested anvil dir");
        fs::write(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/ANVIL.md"),
