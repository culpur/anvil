...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    if let Some(explicit) = std::env::var_os("ANVIL_UPSTREAM") {
    ...
    candidates.push(primary_repo_root.join("reference-source").join("anvil"));
    candidates.push(primary_repo_root.join("vendor").join("anvil"));
