...
use api::{
    resolve_startup_auth_source, AuthSource, AnvilApiClient, ContentBlockDelta, InputContentBlock,
...
use runtime::{
    ...
    ConversationMessage, ConversationRuntime, CronDaemon, LspManager, LspServerConfig,
    McpServerManager, MemoryManager, MessageRole, OAuthAuthorizationRequest, OAuthConfig,
    OAuthTokenExchangeRequest, PermissionMode, PermissionPolicy, ProjectContext, RuntimeError,
    Session, TaskManager, TokenUsage, ToolError, ToolExecutor, UsageTracker,
};
use serde_json::json;
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Starting Anvil OAuth login...");
...
fn run_repl(
    ...
    // Start the cron daemon.  It fires due cron entries in the background.
    // Set the env var ANVIL_NO_CRON=1 to disable it (e.g. in tests).
    ...
    // Monotonic checkpoint: we deliver notifications for tasks that completed
    // *after* this instant, updated each time we process completed tasks.
    let mut task_check_instant = Instant::now();

    loop {
        // Before reading user input, check whether any background tasks
        // finished since the last check and inject notifications.
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        if !has_anvil_md {
            lines.push(
                "  First run        /init scaffolds ANVIL.md, .anvil.json, and local session files"
                    .to_string(),
            );
        }
        lines.join("\n")
...
) -> Result<ConversationRuntime<DefaultRuntimeClient, CliToolExecutor>, Box<dyn std::error::Error>>
{
    ...
    Ok(ConversationRuntime::new_with_features(
        ...
        CliToolExecutor::new(allowed_tools.clone(), emit_output, tool_registry.clone(), mcp_manager, lsp_manager),
...
impl DefaultRuntimeClient {
    fn new(
        ...
        Ok(Self {
            ...
            client: AnvilApiClient::from_auth(resolve_cli_auth_source()?)
    ...
    fn execute(&mut self, tool_name: &str, input: &str) -> Result<String, ToolError> {
        ...
        if tool_name == "AskUserQuestion" {
            ...
            if self.emit_output {
                ...
                let markdown = format_tool_result(tool_name, &summary, false);
                self.renderer
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  anvil login                            Start the OAuth login flow"
    )?;
    writeln!(
        out,
