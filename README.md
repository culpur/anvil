...
## What's New in v2.2.15 — The largest release in Anvil's history

**121 commits over v2.2.13.** v2.2.15 supersedes the corrupted v2.2.14 tag. Two foundational arcs that started in v2.2.5 finish here, the provider catalog grows 6x, Cursor becomes a first-class agent surface, and `/model` reaches across every configured provider in one TAB-completable list.

### `/cursor` first-class command tree

Cursor's public Cloud Agents API is repo-bound and uses a different UX than a typical chat provider, so it gets its own command tree instead of being shoehorned into `/model`. Six TAB-completable subcommands:

- **`/cursor launch <prompt>`** — `POST /v1/agents` against the current workspace's GitHub repo, opens an SSE-streamed agent tab
...
`git remote get-url origin` is mandatory — Cursor errors loudly if the current workspace isn't a GitHub repo. 8-axis capability contract green on all six.

### `/model` cross-provider unified picker

`/model<TAB>` now enumerates **every configured provider's models in one list**, provider-prefixed for clarity:
...
Picking `cursor/claude-4-sonnet-thinking` performs an **atomic switch** — provider routing, system prompt identity, and TUI chrome update together. Unconfigured providers are excluded from the picker. Lists are live-fetched from each provider's `/models` endpoint (cached per session); the static registry is fallback only.
...
- **AWS Bedrock** — hand-rolled SigV4 signing (no AWS SDK = ~50 MB smaller binary), audited against AWS test vectors. `InvokeModel` + `InvokeModelWithResponseStream`.
- **Google Gemini OAuth + Antigravity** — Code Assist OAuth (PKCE flow, dynamic-port localhost callback, atomic token save, pre-emptive refresh). Headers say `User-Agent: Anvil/2.2.15 ({os})` and `x-goog-api-client: anvil-cli` — not VS Code.
- **GitHub Copilot** — device flow.
- **Azure OpenAI** — deployment-name + `api-version` URL pattern.
- **Cursor Cloud Agents** — public API (see `/cursor` above).
- **24 OpenAI-compatible providers** added in one cycle: groq, fireworks, mistral, perplexity, deepseek, togetherai, deepinfra, cerebras, nvidia-nim, huggingface, moonshotai, nebius, scaleway, stackit, baseten, cortecs, 302ai, zai, openrouter, lmstudio, chutes, minimax, opencode, opencode-go. Each gets a dedicated config block and is selectable via `/provider <slug>`.

`/provider login <slug>` runs the right flow for each: API-key paste + live validation for direct-key providers, OAuth PKCE for Google, device flow for Copilot, SigV4 for Bedrock.

### Memory Cohesion Arc — completing v2.2.5

The seven-layer memory system from v2.2.5 (Sensory / Working / Episodic / Semantic / Procedural / Reflective / Long-term) finally cohesion-tests end-to-end:
...
- **Past sessions** (up to ~1,800 JSONL files, ~1 GB) — gated behind `--include-sessions`, summarized via your configured provider into `~/.anvil/daily/` records. Resumable across runs; fails loud if no provider configured.
...
17 features + 7 security/verify items: `ANVIL_PROJECT_DIR` in MCP child env, `ANVIL_SESSION_ID` in Bash subprocess + hooks, `ANVIL_DISABLE_ALTERNATE_SCREEN`, `ANVIL_EFFORT` env + hook field, transcript view nav (`?`, `{`, `}`, `v`), cross-session agent view (`anvil agents` live monitor), `worktree.baseRef` setting, `autoMode.hard_deny` classifier short-circuit, hook `continueOnBlock` for PostToolUse, hook args `string[]` exec form (no shell), `/scroll-speed` with live preview, `/goal` name collision resolution, `anvil plugin details <name>`, `--include-sessions`, subagent OTel headers with parent-agent-id. Plus the security gates: `--resume`/`--continue` underscore paths, plan-mode + Edit allow rule write blocking, MCP content-block result visibility, `Skill(name *)` wildcard prefix matching, settings hot-reload for symlinked files, stream-idle false-fire after stream complete, multi-image paste correctness.
...
Several feedback rules were codified as test-suite-enforced contracts during this cycle:
...
- **CC-only naming** — Anvil docs/code/changelog say "CC", never "Claude Code"
...
Seven targets: macOS arm64 + x86_64, Linux arm64 + x86_64, Windows x86_64, FreeBSD x86_64, NetBSD x86_64. SHA256s published to anvilhub.culpur.net/sha256.

See [RELEASE-NOTES-v2.2.15.md](RELEASE-NOTES-v2.2.15.md) for the full narrative.

---

## What's New in v2.2.14 — Parallel tabs become solid

> **Note:** v2.2.14 was tagged internally but never publicly released — the binaries were never published. v2.2.15 supersedes it and includes everything below plus the v2.2.15-specific headlines. This section is preserved as the historical record of the work that landed in this cycle.
